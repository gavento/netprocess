...
class Installer:
    ...
    def __init__(
        self, version=None, preview=False, force=False, accept_all=False, base_url=BASE_URL,
    ):
    ...
    def get_version(self):
        ...
        if os.path.exists(POETRY_LIB):
            ...
            if not current_version_re:
                print(
                    colorize(
                        "warning", "Unable to get the current Poetry version. Assuming None",
                    )
                )
            else:
...
def main():
    ...
    parser.add_argument("-y", "--yes", dest="accept_all", action="store_true", default=False)
